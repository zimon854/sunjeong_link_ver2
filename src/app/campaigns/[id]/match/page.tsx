  ...
  if (profile.role === 'influencer') {
    ...
    return (
      <div className="max-w-md mx-auto mt-10 p-6 border rounded bg-white text-center">
        ...
        {myMatch ? (
          <div>
            <div>신청 상태: <span className={
              myMatch.status === 'accepted' ? 'text-green-600' :
              myMatch.status === 'rejected' ? 'text-red-600' : 'text-gray-600'}>
              {myMatch.status === 'accepted' ? '수락됨' : myMatch.status === 'rejected' ? '거절됨' : '대기중'}
            </span></div>
          </div>
        ) : (
          <button onClick={handleApply} className="bg-blue-600 text-white px-4 py-2 rounded">참여 신청</button>
        )}
        ...
        {matches.map(m => (
          <div key={m.id} className="flex items-center justify-between border-b py-2">
            <div>
              ...
              <span className={
                m.status === 'accepted' ? 'text-green-600 ml-2' :
                m.status === 'rejected' ? 'text-red-600 ml-2' : 'text-gray-600 ml-2'}>
                {m.status === 'accepted' ? '수락됨' : m.status === 'rejected' ? '거절됨' : '대기중'}
              </span>
            </div>
            <div className="flex gap-2">
              <button onClick={() => handleAccept(m.id)} className="bg-green-500 text-white px-3 py-1 rounded">수락</button>
              <button onClick={() => handleReject(m.id)} className="bg-red-500 text-white px-3 py-1 rounded">거절</button>
            </div>
          </div>
        ))}
      </div>
