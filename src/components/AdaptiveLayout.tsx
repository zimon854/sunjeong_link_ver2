  ...
  if (!deviceInfo) {
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center">
        <div className="pwa-loading w-12 h-12 border-4 border-slate-300/70 border-t-blue-500 rounded-full animate-spin"></div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <main 
        className={`${
          deviceInfo.isPWA 
            ? 'pwa-safe-area min-h-screen bg-slate-50'
      ...
      >
        ...
        {!deviceInfo.isPWA && (
          <div className="web-only-header p-4 text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-full border border-blue-100">
              ...
              <span className="text-slate-600 text-sm font-medium">웹 버전으로 실행 중</span>
              <span className="text-slate-400 text-xs">앱 설치 시 더 나은 경험 제공</span>
            </div>
          </div>
        )}
        ...
        {deviceInfo.isPWA && (
          <div className="pwa-only-indicator fixed top-4 left-4 z-50 flex items-center gap-2">
            ...
            <span className="text-green-700 text-xs font-medium bg-green-50 px-2 py-1 rounded-full border border-green-200">
              네이티브 앱 모드
            </span>
          </div>
        )}
      ...
      {deviceInfo.isPWA && (
        <>
          {/* 상태 표시바 */}
          <div className="fixed top-0 left-0 right-0 h-1 bg-blue-500 z-50"></div>
